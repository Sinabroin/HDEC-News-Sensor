...
### 4.2 라벨 ↔ URL ↔ 페이지 제목 정렬, 오늘의 신규 이슈 (D7-AD-L)

CTA 라벨과 목적지 페이지의 역할이 어긋나지 않도록, 채널별 표현 길이는 달라도 **URL 매핑과
실제 페이지 역할은 동일**하게 맞춘다.

| 역할 | 페이지(파일) | 페이지 제목 | 이메일/Teams 라벨 | Telegram 라벨 |
|---|---|---|---|---|
| 요약 대시보드 | `daily/dashboard-latest.html` | **요약 대시보드** | **요약 대시보드 보기** | **대시보드 보기** |
| 전체 리포트 | `daily/latest.html` | **전체 리포트** (부제 `Executive Daily Brief`) | **전체 리포트 보기** | **상세 리포트 보기** |

- 혼란의 원인이던 `latest.html`의 화면/탭 제목 `Executive Daily Brief`는 **전체 리포트**로
  정렬했다(`build_static_report.py`의 `REPORT_TITLE`·`<h1>`). `Executive Daily Brief`는
  부제·메타로 보존한다(아카이브·검증기 호환). `dashboard-latest.html`은 브랜드 부제에
  **요약 대시보드**를 노출하고 비프로덕션 `PREVIEW` 표기를 운영본에서 제거한다.
- 생성된 `docs/daily/*.html`은 직접 수정하지 않는다 — builder/template를 고치고 live publish
  워크플로(scheduled-live-refresh / telegram-notify)가 재생성·게시한다.
- 이메일/Teams 본문에는 전체 대시보드의 **오늘의 신규 이슈** 박스와 같은 데이터
  (`brief["top_new_issues"]`)를 `[분류] 제목`(최대 5건) 요약 섹션으로 넣는다. 새 데이터를
  만들지 않고 기존 selected new-issue 풀만 재사용하며, 핵심 링크와 중복되는 항목은 본문에서
  한 번만 노출한다. Telegram 본문에는 신규 이슈 섹션을 넣지 않는다(길이 유지).

> **보류(별도 task):** 전체 대시보드의 사업 카테고리 섹션(수주·재무·정책·경쟁) 보강과
> 브랜드·기상/날씨 섹션은 이번 범위가 아니다. 현재 `브랜드`는 설계상 제외 키워드이고
> `기상/날씨`는 데이터 소스가 없어 가짜 데이터를 만들지 않는다 — 소스 결정 후 진행한다.
