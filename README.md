...
"**요약 대시보드 보기**"와 "**전체 리포트 보기**" inline 버튼이 붙고, 각각
...
- repo **Variables**(권장) 또는 Secrets에 `DASHBOARD_URL`이 있으면 메시지에
  "요약 대시보드 보기" 버튼이 붙는다. 없고 `REPORT_URL`이 `/daily/latest.html`이면
  `/daily/dashboard-latest.html`로 파생한다.
- repo **Variables**(권장) 또는 Secrets에 `REPORT_URL`이 있으면 메시지에
  "전체 리포트 보기" 버튼이 붙는다. 로그에는 URL 값 대신
...
4. GitHub 설정 (Secrets/Variables):
   ...
   - `DASHBOARD_URL` = Pages 요약 대시보드 URL ("요약 대시보드 보기" 버튼, 선택)
   - `REPORT_URL` = Pages 리포트 URL ("전체 리포트 보기" 버튼)
   - `TELEGRAM_BOT_USERNAME` **또는** `TELEGRAM_PERSONAL_BOT_URL` = 1:1 봇 deep link 대상
5. 채널 메시지 버튼:
   - **요약 대시보드 보기** → 정적 대시보드(`DASHBOARD_URL`, 없으면 `REPORT_URL`에서 파생)
   - **전체 리포트 보기** → 정적 리포트(`REPORT_URL`)
