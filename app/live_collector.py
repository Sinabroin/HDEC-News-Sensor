...
# ---------------------------------------------------------------------------
# D7-AE-RC1 — 퍼블리셔 원문 URL 최선노력 해석(resolve)
# ---------------------------------------------------------------------------
# 사용자 실사용 QA: "모든 기사에 원문 링크가 없다." 근본원인은 Google News RSS가 주는
# <link>가 실제 퍼블리셔 페이지가 아니라 news.google.com 경유(obfuscated redirect)
# 토큰이라는 것 — 임원이 클릭해도 Google 자체 SPA 셸이 뜨거나(사내망에서 news.google.com이
# 막히면 전부 실패), 실제 기사가 아니다. RSS 응답 자체(<link>)에는 실 URL이 없어(확인됨:
# 서버사이드 30x 리다이렉트 아님, HTML도 JS로만 실URL을 채움) 해석하려면 이미 네트워크를
# 쓰는 이 수집기 안에서 추가 조회가 필요하다.
#
# 방법: Google이 공개적으로 제공하는 저의 redirect 페이지 안의 data-n-a-{id,sg,ts} 값을
# 읽어, Google 자체 batchexecute 데이터 엔드포인트(Fbv4je RPC)에 되돌려 보내면 실제
# 퍼블리셔 URL을 돌려준다 — Google 서버가 자체적으로 응답하는 값이라 위조/우회가 아니다.
...
def _build_decode_request_body(article_id: str, ts: str, sig: str) -> str:
    """디코드 요청 바디를 만든다(순수 함수) — Fbv4je RPC에 넘길 f.req 페이로드."""
    inner = json.dumps(["garturlreq",
        [["X", "X", ["X", "X"], None, None, 1, 1, "US:en", None, 1,
          None, None, None, None, None, 0, 1],
         "X", "X", 1, [1, 1, 1], 1, 1, None, 0, 0, None, 0],
        article_id, int(ts), sig])
    f_req = json.dumps([[[_DECODE_RPC_ID, inner, None, "generic"]]])
    return "f.req=" + urllib.parse.quote(f_req)


def _parse_decode_response(raw: str):
    """batchexecute 응답 원문 → 디코드된 퍼블리셔 URL(순수 함수, 오프라인 테스트용).

    응답은 ")]}'\\n\\n" 접두 + JSON 배열이고, 실제 URL은 그 안의 JSON-인코딩된 문자열
    (2중 인코딩) 안에 있다 — 정규식으로 문자열 안 이스케이프를 다루기보다 표준
    json.loads를 2번 태워 정확히 판독한다(오탐/부분매치 방지). 형식이 안 맞으면 None
    (가짜 URL을 만들지 않는다) — 예외를 던지지 않고 호출자가 처리하게 한다.
    """
    try:
        body = raw.split("\n", 1)[1] if raw.startswith(")]}'") else raw
        outer = json.loads(body)
        inner = json.loads(outer[0][2])
        resolved = inner[1] if len(inner) > 1 else None
        return resolved if isinstance(resolved, str) and resolved.startswith(
            ("http://", "https://")) else None
    except Exception:
        return None


def _decode_google_news_url(gnews_url: str, timeout: float = LINK_RESOLVE_TIMEOUT):
    """Google News RSS redirect URL → 실제 퍼블리셔 기사 URL(최선노력, 실패 시 None).

    네트워크 2회(리다이렉트 셸 페이지 조회 → 디코드 데이터 엔드포인트 조회) — 어떤
    단계든 실패하면 예외를 삼키고 None을 돌려준다(호출자는 기존 aggregator 링크를
    그대로 쓴다). 파싱 자체는 위 순수 함수들이 담당한다(오프라인 단위테스트 가능).
    """
    try:
        req = urllib.request.Request(gnews_url, headers={"User-Agent": "Mozilla/5.0"})
        with urllib.request.urlopen(req, timeout=timeout) as resp:
            shell_html = resp.read(2_000_000).decode("utf-8", errors="replace")
        attrs = _parse_shell_attrs(shell_html)
        article_id, ts, sig = attrs.get("id"), attrs.get("ts"), attrs.get("sg")
        if not (article_id and ts and sig):
            return None

        body = _build_decode_request_body(article_id, ts, sig)
        ...
        with urllib.request.urlopen(req2, timeout=timeout) as resp2:
            raw = resp2.read(200_000).decode("utf-8", errors="replace")
        return _parse_decode_response(raw)
    except Exception:
        # 네트워크/타임아웃/파싱/무엇이든 — 최선노력이라 조용히 실패한다. 기사 자체나
        # 기존 aggregator 링크에는 영향 없음(호출자가 원래 값을 그대로 유지).
        return None
...
def resolve_publisher_urls(rows: list, timeout: float = LINK_RESOLVE_TIMEOUT,
    ...
    attempted = 0
    for row in rows:
        if attempted >= max_items or (time.monotonic() - started) >= deadline:
            break
        url = row.get("url") or ""
...
def fetch_all(timeout: int = DEFAULT_TIMEOUT, sources_path=None,
    ...
    for group in query_groups:
        ...
        if len(results) >= effective_cap:
            break
    # 원문 URL 최선노력 해석(D7-AE-RC1)은 여기서 자동 실행하지 않는다 — fetch_all은 19개
    # verify_*가 커스텀 sources_path/픽스처로 오프라인 호출하는 표면이라, 여기서 자동으로
    # 네트워크를 타면 그 오프라인 계약을 전부 깬다. 실제 프로덕션 진입점(app.collector의
    # live 분기)이 resolve_publisher_urls(results)를 명시적으로 opt-in 호출한다.
    return results
