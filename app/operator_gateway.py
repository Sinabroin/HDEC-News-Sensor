...
D7-AG-5C — origin 모드 발송(telegram/teams)은 GitHub OAuth로 발급한 HttpOnly signed session cookie가
있을 때만 허용한다. collect는 기존처럼 로그인 없이 허용 Origin+rate limit로 유지한다.
...
def authorize(headers=None, origin: str = "", action: str = "") -> dict:
    ...
    if mode == "shared_secret":
        ...
        return {"ok": False, "who": None, "reason": "unauthorized"}
    if mode == "origin":
        # D7-AG-5C 하이브리드 — 허용목록 Origin만으로 **저위험 수집(collect)**은 계속 인가한다.
        # 발송(telegram/teams)은 같은 허용 Origin에서 온 요청이어야 하며, GitHub OAuth session cookie가
        # 유효한 운영자 login을 담고 있을 때만 통과한다. 서버-대-서버(빈 Origin)는 허용하지 않는다.
        o = (origin or "").strip()
        if not (o and o in set(config.OPERATOR_ALLOWED_ORIGINS)):
            return {"ok": False, "who": None, "reason": "unauthorized"}
        if action in _ORIGIN_MODE_ACTIONS:
            return {"ok": True, "who": "origin:" + o, "reason": ""}
        session = operator_auth.session_from_headers(headers)
        if session:
            return {"ok": True, "who": "github:" + session["login"], "reason": ""}
