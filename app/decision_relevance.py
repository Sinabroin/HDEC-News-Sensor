...
HDEC_SIGNAL = [
    ...
    "중대재해", "사망사고", "안전사고", "특별감독", "하자", "품질", "부실시공",
    "조사", "압수수색",
]
# 현대건설이지만 의사결정과 무관(직접 섹션에서 낮춤) — 헬스케어/생활/스포츠/연예.
# P0-D3A: 배구단(원더독스)·프로팀·우승 등 스포츠 구단 기사를 추가한다 ('현대건설 배구단
# 원더독스 우승'은 직접 언급이어도 임원 전략 신호가 아니다).
HDEC_OFFTOPIC = ["헬스케어", "건강", "병원", "스포츠", "축구", "골프", "야구", "배구",
                 "원더독스", "프로팀", "구단", "감독", "선수단", "우승", "리그",
                 "스타랭킹", "박정아", "선수", "연예", "드라마", "웹툰", "예능",
                 "레시피", "맛집", "여행 후기"]

# 단순 분양·견본주택 마케팅 (P0-D3A) — 소비자 대상 PR로, 임원 전략 신호가 아니다.
# '현대건설 힐스테이트 견본주택 개관/청약'은 직접 언급은 맞지만 의사결정 가치가 낮아
# hdec_strategic / hdec_primary_candidate에서 낮춘다 (현대건설 직접 언급 자체는 유지).
HDEC_SALES_PROMO = ["견본주택", "모델하우스", "분양가", "청약", "특별공급", "미분양",
                    "계약률", "개관", "분양 단지", "단지 분양", "분양 일정", "분양 홍보",
                    "사이버 모델하우스", "1순위 마감", "정당계약", "홍보관"]

# 전략 앵커 (P0-D3A) — 이게 함께 있으면 분양/스포츠/생활 단어가 있어도 임원 전략 신호로 본다
# (강등 예외). 수주·도시정비·데이터센터·SMR·제재·해외·재무 등 실제 의사결정 신호.
HDEC_STRATEGIC_ANCHOR = ["수주", "수주잔고", "데이터센터", "smr", "소형모듈원자로", "원전",
                         "도시정비", "재개발", "재건축", "벌점", "제재", "하도급", "불공정",
                         "epc", "플랜트", "해외", "중동", "전환사채", "회사채", "자금조달",
                         "r&d", "연구원", "뉴에너지", "에너지전환", "조직", "통합",
                         "중대재해", "사망사고", "안전사고", "특별감독", "하자",
                         "품질", "부실시공", "소송", "조사", "압수수색"]
...
ORDER_OFFTOPIC = ["여행", "숙소", "맛집", "가입자", "소비자", "이용자", "패션", "쇼핑",
                  "앱", "취업", "채용", "구인", "일자리", "인력", "인재", "고용", "비자"]
...
# 재무·자금조달 / 자본시장 (P0-C1.13 + P0-D3A) — 전환사채·회사채·CB·유상증자·자금조달·
# 신용도·금리·코스피·주가상승·증권·종목 등. 명시적 AI/DC/프로젝트 전략 맥락이 없으면 AI가
# 아니라 자본시장·재무 신호로 본다 ('투자자/현대건설' 언급만으로 AI 금지). 표시 레이어에서는
# 거시경제 레이더가 아니라 [현대건설 직접]/자본·재무로 라우팅한다(override_radar_section 참고).
FINANCE_TOKENS = ["전환사채", "회사채", "cb 발행", "기업어음", "유상증자", "메자닌",
                  "사채 발행", "자금조달", "조달비용", "차입", "만기", "신용등급",
                  "신용도", "유동성", "프로젝트 파이낸싱", "pf", "금리", "투자자", "재무",
                  "주가상승", "주가 상승", "코스피", "증권", "종목"]
...
def _hdec_demoted(text: str) -> bool:
    """현대건설 직접 언급이지만 임원 전략 신호로 보지 않을지 (P0-D3A) — text는 소문자.

    스포츠 구단(원더독스 등)·헬스케어/생활성·단순 분양/견본주택 마케팅은 직접 언급이어도
    의사결정 가치가 낮다. 단 전략 앵커(수주·도시정비·데이터센터·제재·해외·재무 등)가 함께
    있으면 강등하지 않는다 (예: '현대건설 도시정비 분양' → 도시정비 앵커로 유지).

    이로써 '현대건설 직접 언급(is_hdec_direct)'과 '현대건설 임원 전략 신호(strategic/
    primary)'를 분리한다 — 분양 PR·구단 우승 기사가 [현대건설 직접] 상단을 차지하지 않게."""
    if _hits(text, HDEC_STRATEGIC_ANCHOR):
        return False
    return _hits(text, HDEC_OFFTOPIC) or _hits(text, HDEC_SALES_PROMO)


def is_hdec_strategic(title: str) -> bool:
    """현대건설 직접 + 전략/사업/조직/기술/리스크 신호 — 등급 floor 대상.

    제목만 본다 (스니펫 제외 — 오탐 방지). 스포츠·헬스케어/생활성·단순 분양/견본주택
    마케팅은 직접 언급이어도 전략 신호로 보지 않는다 (_hdec_demoted, P0-D3A)."""
    low = (title or "").lower()
    if not is_hdec_direct(title):
        return False
    if _hdec_demoted(low):
        return False
...
def is_hdec_primary_candidate(row: dict) -> bool:
    ...
    P0-D3A: 분양/견본주택 PR·스포츠 구단성 기사는 직접 언급이어도 [현대건설 직접] 상단
    후보로 올리지 않는다 (_hdec_demoted) — 직접 언급(is_hdec_direct) 자체는 유지된다."""
    text = _raw_text(row)
    if not _hits(text, [n.lower() for n in HDEC_NAMES]):
        return False
    if _hdec_demoted(text):
        return False
...
def override_radar_section(base_section, decision: dict) -> str:
    """radar primary section을 의사결정 관점에서 보정한다 (재무 하드 오버라이드).

    raw 제목+스니펫이 재무·자금조달 신호인데 전략 맥락이 없으면(decision['is_finance']),
    AI로 분류된 기사를 AI 레이더에서 뺀다.

    P0-D3A: 재무는 **거시경제(MACRO)가 아니라 other**로 내린다 — 전환사채·CB 등 자본시장
    이벤트가 거시경제 레이더(FX·유가·금리 시장변수)에 섞이던 오탐을 차단한다. 현대건설 재무는
    decision 멤버십으로 [현대건설 직접](hdec_bucket=재무)에 그대로 남고, 거시는 secondary
    라벨로만 표기된다. 진짜 거시 기사(FOMC·금리 시장변수, base==MACRO)는 건드리지 않는다.

    생성 토픽을 오버라이드 입력으로 쓰지 않는다 — is_finance는 raw 제목+스니펫만 본다.
    radar.classify_section 자체는 바꾸지 않는다 (게이트 verifier/스코어 floor 호환 유지)."""
    d = decision or {}
    if base_section == radar.AI and d.get("is_finance"):
        return radar.OTHER
    if (base_section == radar.AI and d.get("supplier_only")
            and not d.get("supplier_order_context")):
        return radar.OTHER
    return base_section
...
def _memberships(row: dict, category=None) -> tuple[set, dict]:
    ...
    if _is_finance(fin_text) and not _hits(fin_text, FINANCE_STRATEGY_CTX):
        ...
        members.discard(AI)
        # 재무는 거시 레이더 primary가 아니다 (P0-D3A) — 다른 멤버십(또는 현대건설 직접)이
        # 있을 때만 거시를 secondary로 붙인다 (현대건설 전환사채 → [현대건설 직접] primary,
        # 거시 secondary). 단독 자본시장 commentary(현대건설/수주 등 없는 'CB 발행 역풍' 류)는
        # 거시 primary로 올리지 않고 other로 남긴다 — 임원 핵심 신호가 아니다.
        if members or is_hdec_direct(title):
            members.add(MACRO)
...
def _reason(primary: str, members: set, flags: dict) -> str:
    ...
    if flags.get("is_finance"):
        ...
        tail = "국내외 발주 환경·수주 경쟁 구도 확인"
    elif primary == RISK:
        tail = "입찰 자격·평판·안전관리 리스크 확인"
    elif primary == AI:
        tail = "데이터센터 EPC·전력 인프라 수주 기회 점검"
    else:
        tail = "참고 신호"
    parts.append(tail)
