...
_BUILDING_HOUSING = TopicProfile(
    ...
    # RC4: 데이터센터 bare 키워드는 AI/전력 렌즈 fan-out을 만들므로 건축주택에서 제외한다.
    # 하자는 사용자 지정 안전·품질 직접 근거이므로 safety_quality에서만 판정한다.
    include_keywords=(
        "국내건축", "주택", "아파트", "힐스테이트", "디에이치", "도시정비", "재건축",
        "재개발", "리모델링", "분양", "미분양", "조합", "공사비",
    ),
...
_PLANT = TopicProfile(
    ...
    # 산업설비/수처리).
    include_keywords=(
        "플랜트", "원전", "LNG", "EPC", "발전소", "정유", "석유화학", "산업설비", "수처리",
    ),
    relevance_anchors=_BIZ_ANCHOR + (
...
_NEW_ENERGY = TopicProfile(
    ...
    # RC4: bare 데이터센터는 AI value-chain 기사 전체를 New Energy로 복제하므로 제외한다.
    # 데이터센터는 전력/냉각 맥락이 함께 있을 때만 이 렌즈의 직접 근거가 된다.
    include_keywords=(
        "SMR", "수소", "재생에너지", "해상풍력", "풍력", "태양광", "ESS", "전력망",
        "송전", "배전", "데이터센터 전력", "데이터센터 냉각", "전력 인프라",
        "전력인프라", "냉각", "에너지 인프라", "탄소중립", "CCUS",
    ),
    relevance_anchors=_BIZ_ANCHOR + (
...
_SAFETY_QUALITY = TopicProfile(
    ...
    # RC4 사용자 지정 강한 근거만 허용한다. bare "안전"은 홍보성 기술 기사까지 잡으므로
    # 제외하고 중대재해/특별감독/산업안전/품질/하자/부실시공/붕괴/사망·안전사고처럼
    # 사건·제재·품질 문제를 직접 나타내는 토큰만 둔다.
    include_keywords=(
        "중대재해", "특별감독", "벌점", "산업안전", "품질", "하자", "부실시공",
        "철근누락", "철근 누락", "붕괴", "사망사고", "안전사고", "산재",
    ),
    relevance_anchors=_BIZ_ANCHOR + (
