...
# D7-AE-RC1 — query-provenance corroboration. 쿼리로 수집됐다는 사실만으로는 매칭을
# 인정하지 않는다(실사용 QA: "파나마 메트로 3호선"에 대구 모노레일 기사, "코즐로두이 원전"에
# 미국 원전시장 기사가 붙는 등 무관 기사가 현장에 매칭됨). 제목/스니펫에 이름을 직접
# 언급하지 않는 query 매칭은, 그 현장명에서 파생한 "식별 토큰"(국가/도시/고유명사 성격의
# 단어)이 제목·스니펫에 함께 있을 때만 인정한다. 공종/업종 단어(원전·플랜트·철도 등)는
# 일부러 제외한다 — business_lens 자체가 이미 그 업종이라 "원전"류 단어만으로는 같은
# 업종의 무관한 다른 프로젝트와 구별할 수 없다(실측: "원전" 하나로는 코즐로두이 오탐을
# 못 걸러낸다 — 반드시 불가리아/코즐로두이 같은 고유 지명이 있어야 한다).
_GENERIC_NAME_TOKENS = {
    "사업", "공사", "현장", "프로젝트", "단지", "센터", "타워", "플랜트", "공장",
    "발전소", "원전", "터널", "교량", "항만", "철도", "도로", "국도", "고속국도",
    "복선전철", "경전철", "지사", "법인", "사무소", "본부", "지점", "지사무소",
    "민간투자사업", "민간투자시설사업", "건설공사", "설계공사", "시공", "리모델링",
    "신축", "증축", "개발", "설계", "구간", "노선", "공구", "지구", "구역", "부두",
    "컨테이너부두", "준설토", "투기장", "확장", "복원", "지하화", "물류", "산업단지",
    "재건축", "재개발", "정비사업", "아파트", "데이터센터", "발전", "EPC", "epc",
}
...
# ---------------------------------------------------------------------------
# query-provenance corroboration (D7-AE-RC1)
# ---------------------------------------------------------------------------

def corroboration_tokens(item: dict) -> list:
    """항목의 name/aliases에서 파생한 "식별 토큰" 목록 — query-only 매칭의 근거용.

    이름/별칭을 공백·구두점으로 쪼갠 뒤, 숫자를 포함한 토큰(공구/호선/호기 등 순번)과
    범용 건설·조직 명사(_GENERIC_NAME_TOKENS — 원전/플랜트/철도 등 업종어, 공사/현장/
    사업 등 접미어)를 제외한다. 남는 토큰은 대개 국가·도시·고유 프로젝트명이라 "이
    현장 얘기가 맞다"는 근거로 쓸 수 있다. 업종어를 일부러 제외하는 이유: business_lens
    자체가 이미 그 업종이라 "원전" 같은 단어 하나로는 같은 업종의 다른 무관 프로젝트와
    구별되지 않는다(코즐로두이 원전 쿼리가 미국 원전시장 기사에 오매칭된 실사례).
    """
    ...
    for name in names:
        for tok in _TOKEN_SPLIT_RE.split(name):
            tok = tok.strip()
            if (len(tok) < 2 or tok in seen or any(ch.isdigit() for ch in tok)
                    or tok in _GENERIC_NAME_TOKENS):
                continue
            seen.add(tok)
            tokens.append(tok)
    return tokens
...
def corroboration_hit(item: dict, text: str) -> str | None:
    """text(제목+스니펫)에 item의 식별 토큰이 하나라도 있으면 그 토큰을, 없으면 None."""
    t = text or ""
    if not t:
        return None
    for tok in corroboration_tokens(item):
        if tok in t:
            return tok
    return None
...
# ---------------------------------------------------------------------------
# 분류 (제목/스니펫 → 사이트 렌즈) — 매칭된 기사에만, 비공개 목록 비노출
# ---------------------------------------------------------------------------

def classify_site_lenses(title, snippet="", path=None) -> list:
    """기사 제목/스니펫이 워치리스트 항목명/별칭을 직접 언급하면 그 항목의 렌즈 매칭을 돌려준다.

    반환: [{id, name, scope, business_lens, matched_via}] (제목 또는 스니펫에 이름이 실제로
    들어 있을 때만). matched_via는 'title'|'snippet'(제목이 우선). 부분문자열 오탐 방지를
    위해 _MIN_MATCH_LEN보다 짧은 이름/별칭은 매칭에 쓰지 않는다. 비공개 항목 전체를
    노출하지 않는다 — 본문이 이미 언급한 항목만 라벨링한다.
    """
    t = title or ""
    s = snippet or ""
    if not t and not s:
    ...
    for item in wl["items"]:
        ...
        if any(len(n) >= _MIN_MATCH_LEN and n in t for n in names):
            via = "title"
        elif any(len(n) >= _MIN_MATCH_LEN and n in s for n in names):
            via = "snippet"
        if via:
            seen_ids.add(item["id"])
            out.append({
