...
# 사용자 지정 뉴스 분류(좌측 목차 흡수) — 서버 렌더 아코디언 키와 1:1.
NEWS_CATS = {"order": "수주", "finance": "재무", "policy": "정책",
             "competitor": "경쟁", "brand": "브랜드", "global_press": "해외 언론"}
...
def check_template() -> None:
    ...
    for key, lbl in NEWS_CATS.items():
        check(f"5: 좌측 목차 뉴스 분류 '{lbl}'(navcat data-acc='{key}')",
              f'class="nav navcat" data-acc="{key}"' in t
              and f'class="nlabel">{lbl}</span>' in t)
    check("6: 뉴스 분류 클릭이 브리핑 <details>를 연다(openNewsCategory→details.acc-sec)",
          "function openNewsCategory" in t and "details.acc-sec" in t and "d.open = on" in t)
    check("6: 좌측 목차 클릭 라우팅이 navcat/navmkt/navwx로 분기",
          'classList.contains("navcat")' in t and 'classList.contains("navmkt")' in t
          and 'classList.contains("navwx")' in t)
...
def check_public_build() -> None:
    with tempfile.TemporaryDirectory(prefix="hdec_railfold_") as tmp:
        ...
        # 6 · 좌측 목차 뉴스 분류 항목(navcat)이 서버 렌더 아코디언 키와 정합
        for key in NEWS_CATS:
            check(f"6: navcat data-acc='{key}'가 아코디언 섹션 키와 정합",
                  f'class="nav navcat" data-acc="{key}"' in html
                  and f'<details class="acc-sec" data-acc="{key}"' in html)
...
def main() -> int:
    ...
    if _failures:
        ...
        for f in _failures:
            print(f"  - {f}")
        return 1
    print("RESULT: PASS — 기존 좌측 목차 단일 진입점(railNav/newsCatFilter 제거 · 뉴스 분류/시장/기상 "
          "흡수) · 운영자 버튼 활성/무설정 fail-closed · 시장 조사 문서 · public-safe (D7-AD-V)")
    return 0
