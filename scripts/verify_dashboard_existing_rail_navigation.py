...
(네트워크/비밀값 0건, 시간 무관). 사용자 D7-AD-V 검증 조건 1–15에 대응한다.
  ...
  9. OPERATOR_API_BASE 주입 시 collect 버튼 enabled · 발송(telegram/teams)은 인증잠금(D7-AG-5B)
 10. 무설정 시 클릭 가능한 Actions/새로고침 링크 · 11. local operator smoke fail-closed
...
def check_public_build() -> None:
    with tempfile.TemporaryDirectory(prefix="hdec_railfold_") as tmp:
        ...
        check("10: 무설정 공개 빌드 — 실행 버튼 3개 visible + 명확한 API 미연결",
              '<summary class="opctl-mode-toggle">' in html
              and 'id="opApiControls"' in html and 'id="opActionLinks"' not in html
...
# ---------------------------------------------------------------------------
# 3 · 운영자 base 주입 빌드 — collect 활성 + 발송 인증잠금 + endpoint 주입 (조건 9)
# ---------------------------------------------------------------------------
...
def check_operator_base_build() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="hdec_railfold_op_") as tmp:
        ...
        if not check("운영자 base 주입 빌드 동작", proc.returncode == 0 and out.exists(),
            ...
            return
        html = out.read_text(encoding="utf-8")
        # 하이브리드(D7-AG-5B): base 주입 빌드는 저위험 collect만 활성화하고, 발송(send/teams)은
        # 인증 필요(auth-locked) 상태로 유지한다 — 공개 Origin 인가로는 발송을 열지 않는다.
        check("9: base 주입 시 collect 활성 + 발송(send/teams) 인증잠금",
        ...
        check("9: 세 Operator endpoint 주입(collect/send/send-teams)",
              '"/api/operator/collect"' in html and '"/api/operator/send"' in html
              and '"/api/operator/send-teams"' in html)
        check("9: 연결형 버튼은 Operator API POST, privileged GitHub API 직접 호출 없음",
              "fetch(base + path" in html and "api.github.com" not in html)
...
def check_discovery_doc() -> None:
    ...
    # D7-AE-RC3 — '사용자 링크 재요청 필요'는 해소됐다: 확인된 repo와 실연동 조사
    # 문서(HORMUZ_LIVE_INTEGRATION.md)가 기록돼야 한다(과거 결론으로 되돌아가면 FAIL).
    check("13: 호르무즈 — RC3 해소 기록(확인된 repo + 실연동 조사 문서 연결)",
          "호르무즈 해협 API" in integration and "후보:" in integration
          and "hormuz-ship-tracker" in integration
          and "HORMUZ_LIVE_INTEGRATION.md" in integration)
