...
(네트워크/비밀값 0건, 시간 무관). 사용자 D7-AD-V 검증 조건 1–15에 대응한다.
 ...
 10. 무설정 시 실행 버튼 0건(+ '운영 API 설정 필요' 한 줄 — D7-AE-RC3 B안) · 11. local operator smoke fail-closed
...
def check_public_build() -> None:
    with tempfile.TemporaryDirectory(prefix="hdec_railfold_") as tmp:
        ...
        # 10 · D7-AE-RC3(B안): 무설정 공개 빌드는 실행 버튼을 아예 렌더하지 않고
        # '운영 API 설정 필요' 한 줄만 남긴다(비활성 버튼을 제품 기능처럼 남기지 않는다).
        check("10: 무설정 공개 빌드 — 실행 버튼 0건 + '운영 API 설정 필요' 한 줄",
              'id="opCollectBtn"' not in html and 'class="opctl-btn' not in html
              and 'id="opPin"' not in html and "운영 API 설정 필요" in html
              and 'id="opctl"' in html)
