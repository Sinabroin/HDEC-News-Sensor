...
def _signal_rank_key(sig, immediate_basis: str = "") -> tuple:
    """Live source-priority first, then the established executive order.

    Mock rows keep effective rank zero, preserving deterministic fixture ordering.
    """
    basis_rank = 1 if immediate_basis == "strict_instant" or _is_strict_immediate(sig) else 0
    is_live = "live" in str(sig.get("signal_origin") or "").lower()
    source_rank = int(sig.get("source_priority_rank") or 0) if is_live else 0
    return (
        source_rank,
        -_score_value(sig),
