...
OPTIONAL_VERIFIERS = [
    ...
    "verify_mobile_layout_structure.py",
...
def check_committed_report() -> None:
    ...
    if check("docs/daily/dashboard-latest.html 존재 (요약 대시보드 스냅샷)",
        ...
        exact_news_censor = (
            'id="article-data"' in dash
            and 'id="pane-articles" class="pane active"' in dash
            and "NEWS CENSOR" in dash
        )
        check("dashboard-latest는 latest.html을 대체하지 않음",
              dash != latest
              and (
                  exact_news_censor
                  or "dashboard-export:summary" in dash
              )
              and 'id="article-data"' not in latest
        ...
        check("dashboard-latest가 preview-model 요약 대시보드 구조 포함",
              exact_news_censor
              or ('id="preview-model"' in dash and "HDEC Executive Radar" in dash))
