...
채널 일일 브리프 메시지에 "오늘 브리프 보기"(리포트)와 "개인 질의하기"(1:1 봇 deep link)
버튼이 정직하고 안전하게 붙는지 결정적으로 검사한다 (네트워크 0건, 비밀값 0건):
...
def check_sender_source() -> None:
    ...
    check("sender에 리포트 버튼 라벨 존재", REPORT_BUTTON_TEXT in src)
...
def check_username_configured() -> None:
    proc = run_payload(TELEGRAM_BOT_USERNAME=SAMPLE_USERNAME,
                       REPORT_URL=SAMPLE_REPORT_URL, TELEGRAM_BOT_TOKEN=FAKE_TOKEN)
    out = (proc.stdout or "")
    check("username 설정 → dry-run-payload 동작 (exit 0)", proc.returncode == 0,
          (proc.stderr or "").strip()[-160:])
    check("username 설정 → 리포트 버튼 노출", REPORT_BUTTON_TEXT in out)
...
def check_not_configured_omits_button() -> None:
    ...
    check("미설정 → 리포트 버튼은 유지", REPORT_BUTTON_TEXT in out)


def check_bad_username_disabled() -> None:
    proc = run_payload(TELEGRAM_BOT_USERNAME="bad name!", REPORT_URL=SAMPLE_REPORT_URL)
    out = proc.stdout or ""
