...
def check_brief_radar() -> None:
    ...
    hdec = brief.get("hdec_direct_signals")
    comp = brief.get("competitor_supply_signals")
    for key, val in [("hdec_direct_signals", hdec), ("ai_radar_signals", ai),
                     ("risk_regulation_signals", risk), ("business_signals", biz),
...
def check_digest_ai_first() -> None:
    ...
    check("digest 거시경제 안내가 전체 리포트 위임 표현",
          "상세 리포트 보기" in msg and "거시경제" in msg)
